...
def evaluate(cfg: DictConfig) -> None:
    ...
    if use_wandb_artifact:
        ...
        model, metadata = load_model_from_artifact(
            artifact_name, wandb_project, wandb_entity
        )
        num_classes = metadata.get("output_dim", 6)
    elif use_wandb_registry:
        registry_model_name = cfg.get(
            "wandb_registry_model_name", "red_wine_quality_model"
        )
        ...
        model, metadata = load_model_from_registry(
            registry_model_name, registry_version, wandb_entity
        )
        num_classes = metadata.get("output_dim", 6)
    else:
        ...
        model.load_state_dict(
            torch.load(model_path / model_name, map_location=DEVICE, weights_only=True)
        )
        metadata = {}
    ...
    if use_wandb_artifact or use_wandb_registry:
        ...
        if metadata:
            print(
                f"Training validation accuracy: {metadata.get('final_val_accuracy', 'N/A')}"
            )
            print(
                f"Training validation loss: {metadata.get('final_val_loss', 'N/A')}\n"
            )
