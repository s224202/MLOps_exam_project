...
class WineData(Dataset):
    ...
    def __init__(self, data_path: Path, download: bool = False) -> None:
        ...
        if not self.data_path.exists():
            if download:
                print(
                    f"Data file not found at {self.data_path}. Downloading from UCI ML Repository..."
                )
...
def preprocess(
    ...
    output_folder: Path = typer.Option(
        Path("data/processed/"), help="Output folder for processed data"
    ),
    ...
    train_val_split_ratio: float = typer.Option(
        0.9, help="Train/validation split ratio"
    ),
) -> None:
