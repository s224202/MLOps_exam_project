...
def visualize(
    ...
    train_dataloader = torch.utils.data.DataLoader(train_set, batch_size=cfg.training.batch_size, shuffle=False)
    test_dataloader = torch.utils.data.DataLoader(test_set, batch_size=cfg.training.batch_size, shuffle=False)
    ...
    ax4.axhline(y=np.mean(per_class_acc), color='r', linestyle='--', 
    ...
    bars = ax5.bar(['Train', 'Test'], [train_acc, test_acc], 
    ...
    # Save figure
    fig.savefig(project_root / cfg.figure_path / cfg.figure_visualization, bbox_inches='tight') # dpi=150, 
    print(f"Visualizations saved to {project_root / cfg.figure_path / cfg.figure_visualization}")
