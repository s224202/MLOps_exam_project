...
def test_get_model_predictions_batch_consistency(sample_model):
    ...
    preds_b4, labels_b4 = get_model_predictions(
        sample_model, loader_batch4, device="cpu"
    )
    preds_b8, labels_b8 = get_model_predictions(
        sample_model, loader_batch8, device="cpu"
    )
...
def test_evaluation_visualizations_with_imbalanced_data():
    ...
    train_preds = np.concatenate(
        [np.zeros(50, dtype=int), np.ones(30, dtype=int), np.full(20, 2, dtype=int)]
    )
    ...
    test_preds = np.concatenate(
        [np.zeros(25, dtype=int), np.ones(15, dtype=int), np.full(10, 2, dtype=int)]
    )
